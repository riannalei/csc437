...
router.get("/:userid", (req, res) => {
  ...
  import_traveler_svc.default.get(userid).then((traveler) => {
    if (traveler) {
      res.json(traveler);
    } else {
...
router.put("/:userid", (req, res) => {
  const { userid } = req.params;
  const newTraveler = req.body;
  import_traveler_svc.default.update(userid, newTraveler).then((traveler) => res.json(traveler)).catch((err) => res.status(404).end());
});
